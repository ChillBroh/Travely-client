  ...
  const salaryDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${API_BASE_URL}/salary/delete/${id}`)
          .then((res) => {
          ...
          <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              {data
                .filter(
                  (Salary) =>
                    Salary.Name && Salary.Name.toLowerCase().includes(query)
                )
                .map((Salary) => (
                  <tr
                    key={Salary._id}
                  ...
                  >
                    <td class="px-6 py-4">{Salary.Name}</td>

                    <td class="px-6 py-4">{Salary.Eid}</td>
                    <td class="px-6 py-4">{Salary.Nic}</td>
                    <td class="px-6 py-4">{Salary.JobPosition}</td>

                    <td class="px-6 py-4">{Salary.Numberaofdates}</td>

                    <td class="px-6 py-4">{Salary.OtHours}</td>
                    <td class="px-6 py-4">{Salary.BasicSalary}</td>
                    <td class="px-6 py-4">{Salary.NetSalary}</td>

                    <td class="px-6 py-4">
                      <button
                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline "
                        onClick={() => salaryDelete(Salary._id)}
                      >
                        Delete{" "}
                      </button>
                    </td>
