...
function SalaryCal() {
  ...
  useEffect(() => {
    if (
      ...
      const otRate = 5;
      const netSalary =
        salaryRate * parseInt(Numberaofdates, 10) +
        otRate * parseInt(OtHours, 10);
  ...
  const handleSubmit = (e) => {
    ...
    const filteredData = newdata.some(
      (employee) =>
        employee.Eid == Eid &&
    ...
    Swal.fire({
      ...
      if (result.isConfirmed && filteredData) {
        if (
          ...
          axios
            .post(`${API_BASE_URL}/salary/add`, newSalary)
            .then(() => {
              Swal.fire(
                "Employee Salary has been successfully Saved!",
                "",
                "success"
              );
